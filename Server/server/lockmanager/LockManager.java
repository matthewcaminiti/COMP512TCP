		...
		for (int i = 0; i < size; i++)
		{
			...
			{    
				...
				else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
				{
					...
					if(l_dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_READ){ //transaction already has READ lock
						bitset.set(0, true);
					}else if(l_dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE){ //transaction already has WRITE lock
						throw new RedundantLockRequestException(dataLockObject.getXId(), "redundant WRITE lock request: Already has WRITE lock");
					}
					//TODO: Lock conversion
				}
			} 
